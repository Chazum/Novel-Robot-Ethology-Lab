...
bool is_above_mating_threshold (int threshold); //return true if sum of left and right IR sensors is above mating threshold
...
bool is_above_max_photo_differential(int threshold);
bool is_above_min_photo_differential(int threshold);
...
int mating_threshold = 600; //how close the robot should be to the object before it triggers mating dance
int max_photo_threshold = 600; //the sum between photo sensor readings has to be above this for seek
int min_photo_threshold = 200;
int photo_threshold = 15;
//Light threshold change
float photo_max = 1000.0; //approximate max possible photo reading (set from observation)
...
//happy dance
		if(is_above_max_photo_differential(max_photo_threshold)){
			happy_dance();
			}
		else if(is_above_min_photo_differential(min_photo_threshold)){
			if(checker == false){
				if (is_above_mating_threshold(mating_threshold)){
					mating_dance();
					checker = true;
					msleep(1000); 
					}					
...
/******************************************************/
bool is_above_max_photo_differential(int threshold){
int photo_sum = left_photo_value + right_photo_value; //get the sum between the photo values
return (abs(photo_sum) > threshold); //returns true if the absolute sum between photo sensors is greater than the threshold, otherwise false
}
/******************************************************/
bool is_above_min_photo_differential(int threshold){
int photo_sum = left_photo_value + right_photo_value; //get the sum between the photo values
return (abs(photo_sum) > threshold);
}
/******************************************************/
...
/******************************************************/
bool is_above_mating_threshold(int threshold){
int distance_sum = left_ir_value + right_ir_value; //get sum between IR values
return (abs(distance_sum)> threshold);
}	
/******************************************************/
bool is_front_bump(){
...
if(abs(photo_sum) > max_photo_threshold){
drive(0.9, 0.1, 5.0);
msleep(5000);
drive(0.0, 0.0, 1000);
}
